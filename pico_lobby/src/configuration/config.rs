...
mod tests {
    ...
    #[test]
    fn default_config_includes_server_entry_slot() {
        let toml = toml::to_string_pretty(&Config::default()).unwrap();

        // Each default server entry must carry an explicit slot so the
        // autogenerated config documents the field rather than hiding it.
        assert!(toml.contains("slot = 0"));
    }

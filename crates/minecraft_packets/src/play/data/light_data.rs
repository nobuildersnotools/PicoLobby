...
/// Light data for V1_16 (protocols 735–754): masks are a single VarInt bitmask, not BitSet.
#[derive(PacketOut, Default)]
pub struct LightDataLegacy {
...
impl LightDataLegacy {
    /// V1_16 worlds are 256 blocks tall (16 sections) plus 2 boundary sections = 18 total.
    const TOTAL_SECTIONS: u32 = 18;

    pub fn new_void() -> Self {
        let mask = (1i32 << Self::TOTAL_SECTIONS) - 1;
        let full_sky = Light::new(vec![0xFFu8 as i8; 2048]);
        let no_block = Light::new(vec![0i8; 2048]);
        Self {
            sky_light_mask: VarInt::new(mask),
            block_light_mask: VarInt::new(mask),
            empty_sky_light_mask: VarInt::new(0),
            empty_block_light_mask: VarInt::new(0),
            sky_light_arrays: LengthPaddedVec::new(vec![
                full_sky;
                Self::TOTAL_SECTIONS as usize
            ]),
    ...
    pub fn from_light_data(
        ...
        let mut sky_light_arrays = Vec::with_capacity(Self::TOTAL_SECTIONS as usize);
        ...
        sky_light_arrays.push(Light::new(vec![0xFFu8 as i8; 2048]));
        while sky_light_arrays.len() < Self::TOTAL_SECTIONS as usize {
            sky_light_arrays.push(Light::new(vec![0xFFu8 as i8; 2048]));
        }

        let mut block_light_arrays = Vec::with_capacity(Self::TOTAL_SECTIONS as usize);
        ...
        block_light_arrays.push(Light::new(vec![0i8; 2048]));
        while block_light_arrays.len() < Self::TOTAL_SECTIONS as usize {
            block_light_arrays.push(Light::new(vec![0i8; 2048]));
        }
